...
| Entity | Purpose |
| --- | --- |
| `sensor.<setup_slug>_<battery_name>_action` | Current planned action: `charge`, `discharge`, or `hold`. Includes attributes such as `next_action` and `next_action_timestamp`. |
...
| Entity | Purpose |
| --- | --- |
| `sensor.<setup_slug>_<comfort_name>_action` | Current planned action: `on` or `off`. Includes attributes describing the next action change. |
